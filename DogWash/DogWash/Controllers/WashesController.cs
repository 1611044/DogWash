...
namespace DogWash.Controllers
{
    public class WashesController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ID,apptime,owner,pet,washtype")] Wash wash)
        {
            string num ="";
            wash.owner = User.Identity.GetUserId();
            if (ModelState.IsValid)
            {
               foreach (Owner o in db.Owners)
                {
                    if(wash.owner == o.owner)
                    {
                        num = o.phonenumber;
                    }
                }
                ...
                String Message = "Youhaveregisteredsuccessfuly.";
                String url = "https://api.zipwhip.com/message/send?session=1859f80b2-5576-4a8e-b43a-ef44661d6803:345142302&contact=" + justDigits + "&body=" + Message;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,apptime,owner,pet,washtype")] Wash wash)
        {
            if (ModelState.IsValid)
            {
